    ...
    return function (cubelet) {
        ...
        cubelet.faces.forEach(function (face) {
            ...
            //  and also allow text to be placed on it.
                ...
                // stickerElement.classList.add('highlight');
                stickerElement.classList.add(face.color.name);
                face.element.appendChild(stickerElement);

                // var brightStickerElement = document.createElement( 'div' );
                // brightStickerElement.classList.add( 'bright' );
                // brightStickerElement.classList.add( face.color.name );
                // face.element.appendChild( brightStickerElement );
...
ERNO.renderers.CSS3DCubelet.methods = (function () {
    ...
    return {
        ...
        allStickersAreHidden: function () {
            return this.getFaceElements(' .sticker').every(function (item) {
                return item.style.display === "none"
            });
        },
        isHiddenFace: function (face) {
            return this.getFaceElements(face.charAt(0) + face.slice(1).toLowerCase() + ' .sticker').every(function (item) {
                return item.style.display === "none"
            });
        },
        ...
        toggleStickers: function () {
            if (this.allStickersAreHidden())
                this.showStickers()
            else
                this.hideStickers()
        },
        toggleFaceSticker: function (face) {
            if (this.isHiddenFace(face))
                this.showFaceSticker(face)
            else
                this.hideFaceSticker(face)
        },
        showFaceSticker: function (face) {
            this.getFaceElements(face.charAt(0) + face.slice(1).toLowerCase() + ' .sticker').forEach(showItem);
        },

        hideFaceSticker: function (face) {
            this.getFaceElements(face.charAt(0) + face.slice(1).toLowerCase() + ' .sticker').forEach(hideItem);
        },
        ...
        highlight: function () {
            this.getFaceElements(' .sticker').forEach(function (item) {
